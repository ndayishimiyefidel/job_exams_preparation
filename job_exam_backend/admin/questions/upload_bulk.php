...
if (!in_array($file['type'], $allowed_types)) {
    sendErrorResponse('Invalid file type. Please upload CSV or Excel file.');
}
...
if (($handle = fopen($filepath, "r")) !== FALSE) {
    ...
    while (($data = fgetcsv($handle)) !== FALSE) {
        if (count($data) >= 8) { // exam_id, question_text, choice_a, choice_b, choice_c, choice_d, choice_e, correct_answer
